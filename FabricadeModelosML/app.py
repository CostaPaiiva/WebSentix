...
# Define a função que lida com a requisição de comparação de versões
def comparar_versoes(projeto, v1, v2):
    import os  # Importa o módulo os para interagir com o sistema operacional
    import json  # Importa o módulo json para trabalhar com dados JSON

    # Constrói o caminho base para as pastas de treino das versões
    ...
    # Verifica se um dos arquivos meta.json não existe
    if not os.path.exists(p1) or not os.path.exists(p2):
        # Retorna uma mensagem de erro se uma das versões não for encontrada
        return "Uma das versões não existe", 404

    # Abre o arquivo meta.json da primeira versão em modo de leitura
    with open(p1, encoding="utf-8") as f:
        m1 = json.load(f)  # Carrega os dados JSON do arquivo para m1

    # Abre o arquivo meta.json da segunda versão em modo de leitura
    with open(p2, encoding="utf-8") as f:
        m2 = json.load(f)  # Carrega os dados JSON do arquivo para m2
    ...
    if os.path.exists(caminho_prod):  # Verifica se o arquivo PRODUCAO.txt existe
        ...
        with open(caminho_prod, encoding="utf-8") as f:
            producao = f.read().strip()  # Lê o conteúdo do arquivo e remove espaços em branco

    # Diferença de score
    try:  # Inicia um bloco try para tratamento de erros
        # Calcula a diferença entre os melhores scores das duas versões
        diff = float(m2["melhor_score"]) - float(m1["melhor_score"])
    except:  # Captura qualquer exceção que ocorra
        diff = None  # Se ocorrer um erro, define diff como None

    return render_template(  # Renderiza o template comparar_versoes.html
        "comparar_versoes.html",  # Nome do template
        projeto=projeto,  # Passa o nome do projeto para o template
        v1=v1,  # Passa o nome da primeira versão para o template
        v2=v2,  # Passa o nome da segunda versão para o template
        m1=m1,  # Passa os metadados da primeira versão para o template
        m2=m2,  # Passa os metadados da segunda versão para o template
        producao=producao,  # Passa a versão em produção para o template
        diff=diff  # Passa a diferença de score para o template
    )


# Define a rota para a página de comparação de duas versões de um projeto
